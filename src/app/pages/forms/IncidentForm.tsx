...
function Sel({ name, defaultValue, children }: { name: string; defaultValue?: string; children: React.ReactNode }) {
  return (
...
export default function IncidentForm({ open, onClose, onSaved, incident, currentUser }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('lists').select('row_id,failed_component,failure_type').or('failed_component.not.is.null,failure_type.not.is.null'),
  ...
  useEffect(() => {
    if (!custId) { setDistricts([]); setDistId(''); return; }
  ...
  const handleImageUpload = async (file: File, slot: 1 | 2) => {
    ...
    try {
      ...
      const filePath = `incidents/img-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
      const { error } = await supabase.storage.from('Native Files').upload(filePath, file);
      if (error) throw error;
      const { data: { publicUrl } } = supabase.storage.from('Native Files').getPublicUrl(filePath);
      setUrl(publicUrl);
...
so_number:    fd.get('so_number')    ?? null,
    ...
    if (!editing) payload.event_id = fd.get('event_id') || '';

    try {
      const { error } = editing
        ? await supabase.from('incidents').update(payload).eq('row_id', incident.row_id)
  ...
  const uniqueFailureTypes = [...new Map(
    listItems.filter(l => l.failure_type).map(l => [l.failure_type, l])
  ).values()];
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{editing ? `Edit Incident #${incident.event_id}` : 'Report New Incident'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-x-6 gap-y-4 mt-2">
          ...
          <F label="Incident Date" required>
            <Input name="date_incident" type="date" defaultValue={incident?.date_incident || ''} required />
          </F>

          <F label="Status">
            <Sel name="incident_status" defaultValue={incident?.incident_status || 'Open'}>
              <option value="">— Select —</option>
              {INCIDENT_STATUS_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>

          <F label="Severity">
            <Sel name="incident_severity" defaultValue={incident?.incident_severity || ''}>
              <option value="">— Select —</option>
              {INCIDENT_SEVERITY_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>
          ...
          <F label="XC Caused">
            <Sel name="xc_caused" defaultValue={incident?.xc_caused || ''}>
              <option value="">— Select —</option>
              {XC_CAUSED_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>

          <F label="Event Category">
            <Sel name="event_category" defaultValue={incident?.event_category || ''}>
              <option value="">— Select —</option>
              {EVENT_CATEGORY_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>

          <F label="Field or Facility">
            <Sel name="field_facility" defaultValue={incident?.field_facility || 'Field'}>
              <option value="">— Select —</option>
          ...
          <F label="Report Version">
            <Sel name="report_version" defaultValue={incident?.report_version || 'Preliminary'}>
              <option value="">— Select —</option>
              {REPORT_VERSION_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>
          ...
          <F label="Customer" required>
            <select value={custId} onChange={e => { setCustId(e.target.value); setDistId(''); }}
          ...
          <F label="Operating Company">
            <Sel key={`op-${incident?.row_id}-${epCompanies.length}`} name="operating_company" defaultValue={incident?.operating_company || ''}>
              <option value="">— Select —</option>
          ...
          <F label="XC Rep">
            <Sel key={`xcrep-${incident?.row_id}-${sqmReps.length}`} name="xc_rep" defaultValue={incident?.xc_rep || currentUser?.name || ''}>
              <option value="">— Select —</option>
          ...
          <F label="XC District">
            <Input name="xc_district" defaultValue={incident?.xc_district || ''} placeholder="e.g. Permian Basin" />
          </F>
          ...
          <F label="Product Line">
            <Sel name="product_line" defaultValue={incident?.product_line || ''}>
              <option value="">— Select —</option>
              {PRODUCT_LINE_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>

          <F label="Firing System">
            <Sel name="firing_system" defaultValue={incident?.firing_system || ''}>
              <option value="">— Select —</option>
              {FIRING_SYSTEM_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>

          <F label="Failed Component">
            <Sel key={`fc-${incident?.row_id}-${listItems.length}`} name="failed_component" defaultValue={incident?.failed_component || ''}>
              <option value="">— Select —</option>
              {listItems.filter(l => l.failed_component).map(l => (
                <option key={l.row_id} value={l.row_id}>{l.failed_component}</option>
              ))}
            </Sel>
          </F>

          <F label="Failure Type">
            <Sel key={`ft-${incident?.row_id}-${listItems.length}`} name="failure_type" defaultValue={incident?.failure_type || ''}>
              <option value="">— Select —</option>
              {uniqueFailureTypes.map(l => (
                <option key={l.row_id} value={l.row_id}>{l.failure_type}</option>
              ))}
            </Sel>
          </F>

          <F label="Vendor">
            <Sel key={`vnd-${incident?.row_id}-${vendors.length}`} name="vendor" defaultValue={incident?.vendor || ''}>
              <option value="">— Select —</option>
              {vendors.map(v => <option key={v.row_id} value={v.row_id}>{v.vendor}</option>)}
            </Sel>
          </F>

          <F label="Vendor Caused">
            <Sel name="vendor_caused" defaultValue={incident?.vendor_caused || ''}>
              <option value="">— Select —</option>
              {VENDOR_CAUSED_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>
          ...
          <div className="col-span-2">
            <F label="Investigation">
              <Textarea name="investigation" rows={3}
          ...
          <F label="Action Assigned To">
            <Sel key={`aa-${incident?.row_id}-${sqmReps.length}`} name="action_assigned_to" defaultValue={incident?.action_assigned_to || ''}>
              <option value="">— Select —</option>
              {sqmReps.map(r => <option key={r} value={r}>{r}</option>)}
            </Sel>
          </F>
          ...
          <F label="Action Due Date">
            <Input name="action_due_date" type="date" defaultValue={incident?.action_due_date || ''} />
          </F>

          <F label="Action Status">
            <Sel name="action_status" defaultValue={incident?.action_status || ''}>
              <option value="">— Select —</option>
              {ACTION_STATUS_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
            </Sel>
          </F>
          ...
          <F label="Closed Date">
            <Input name="closed_date" type="date" defaultValue={incident?.closed_date || ''} />
          </F>

          <F label="Closed By">
            <Sel key={`cb-${incident?.row_id}-${sqmReps.length}`} name="closed_by" defaultValue={incident?.closed_by || ''}>
              <option value="">— Select —</option>
              {sqmReps.map(r => <option key={r} value={r}>{r}</option>)}
            </Sel>
          </F>
          ...
          {([1, 2] as const).map(slot => {
            ...
            return (
              <div key={slot}>
                <Label className="text-xs font-semibold text-gray-600 mb-2 block">Image {slot}</Label>
                {url ? (
                  <div className="relative inline-block group">
                    <img src={url} alt={`Evidence ${slot}`}
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-gray-400 transition-colors">
                    <input type="file" accept="image/*" className="hidden"
                      disabled={uploading}
                      onChange={e => { const f = e.target.files?.[0]; if (f) handleImageUpload(f, slot); }} />
                    {uploading ? (
